...
export function AdminPricing() {
  ...
  const load = async () => {
    ...
    const [cfgRes, discRes, evtRes] = await Promise.all([
      ...
      supabase
        .from("special_events")
        .select("id, name, ticket_price, requires_payment, event_date, status")
  ...
  const toggleEventPayment = async (id: string, current: boolean) => {
    ...
    const { error } = await supabase
      .from("special_events")
      .update({ requires_payment: next })
      .eq("id", id);
    if (error) toast.error(error.message);
    else {
      toast.success(next ? "Payment required" : "Free event");
      load();
    }
  };
  ...
  const packages: Record<string, { name: string; capacity: string; price: number }> =
    config.packages ?? {};
  const parkingOptions: Array<{ id: string; name: string; price: number }> =
    config.parking_options ?? [];
  const depositCfg: { type: string; value: number; min_amount: number } = config.deposit ?? {
    type: "percentage",
    value: 30,
    min_amount: 500,
  };
    ...
    <div className="space-y-6" ref={contentRef}>
      <header>
        ...
        <p className="text-sm text-foreground/60 mt-1">
          Manage all prices, packages, parking, deposits, discounts, and event payment requirements.
        </p>
      </header>
      ...
      {tab === "packages" && (
        <div className="glass rounded-3xl p-5 sm:p-6">
          ...
          <p className="text-xs text-foreground/50 mb-5">
            Edit the names, capacities, and prices of reservation packages shown on the /reserve
            page.
          </p>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {Object.entries(packages).map(([key, pkg]) => (
              <div
                key={key}
                className="glass rounded-2xl p-4 border border-border/30 overflow-hidden"
              >
                ...
                <div className="flex gap-2 mb-2 min-w-0">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const next = {
                        ...packages,
                        [key]: { ...pkg, price: Number(e.target.value) },
                      };
                      setConfig((s) => ({ ...s, packages: next }));
                    }}
      ...
      {tab === "parking" && (
        <div className="glass rounded-3xl p-5 sm:p-6">
          ...
          <p className="text-xs text-foreground/50 mb-5">
            Edit parking option names and prices shown on the /reserve page.
          </p>
            ...
            {parkingOptions.map((opt, idx) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="text-xs uppercase tracking-wider text-gold/70 truncate">
                    {opt.id}
                  </div>
                  <button
                    onClick={() => {
          ...
          <div className="space-y-4">
            <label className="block">
              ...
              <select
                ...
                onChange={(e) =>
                  setConfig((s) => ({ ...s, deposit: { ...depositCfg, type: e.target.value } }))
                }
            ...
            <label className="block">
              <span className="eyebrow">
                {depositCfg.type === "percentage" ? "Percentage (%)" : "Amount (KES)"}
              </span>
              <input
                name="deposit-value"
                inputMode="numeric"
                ...
                onChange={(e) =>
                  setConfig((s) => ({
                    ...s,
                    deposit: { ...depositCfg, value: Number(e.target.value) },
                  }))
                }
                className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
              />
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(e) =>
                  setConfig((s) => ({
                    ...s,
                    deposit: { ...depositCfg, min_amount: Number(e.target.value) },
                  }))
                }
                className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
              />
      ...
      {tab === "discounts" && (
        <div className="space-y-4">
          <button
            onClick={() => {
              setEditingDiscount(null);
              setDiscountForm(DISCOUNT_EMPTY);
              setShowDiscountForm(true);
            }}
            ...
            <div className="glass rounded-3xl p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div className="font-display text-xl">
                  {editingDiscount ? "Edit" : "New"} Discount
                </div>
                <button
                  onClick={() => {
                    setShowDiscountForm(false);
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(e) => setDiscountForm((s) => ({ ...s, name: e.target.value }))}
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
                  />
                </label>
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDiscountForm((s) => ({ ...s, code: e.target.value.toUpperCase() }))
                    }
                    placeholder="EMPIRE20"
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold uppercase tracking-wider"
                  />
                </label>
                <label className="block">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDiscountForm((s) => ({ ...s, type: e.target.value as any }))
                    }
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
                  >
                ...
                <label className="block">
                  <span className="eyebrow">
                    {discountForm.type === "percentage" ? "Percentage off" : "Amount off (KES)"}
                  </span>
                  <input
                    name="discount-value"
                    inputMode="numeric"
                    value={discountForm.value}
                    onChange={(e) =>
                      setDiscountForm((s) => ({ ...s, value: Number(e.target.value) }))
                    }
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
                  />
                </label>
                <label className="block">
                  ...
                  <select
                    name="discount-applies-to"
                    value={discountForm.applicable_to}
                    onChange={(e) =>
                      setDiscountForm((s) => ({ ...s, applicable_to: e.target.value as any }))
                    }
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
                  >
                ...
                <label className="block">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDiscountForm((s) => ({ ...s, max_uses: Number(e.target.value) }))
                    }
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
                  />
                </label>
                <label className="block">
                  ...
                  <input
                    name="discount-starts-at"
                    type="datetime-local"
                    value={discountForm.starts_at}
                    onChange={(e) => setDiscountForm((s) => ({ ...s, starts_at: e.target.value }))}
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
                  />
                </label>
                <label className="block">
                  ...
                  <input
                    name="discount-ends-at"
                    type="datetime-local"
                    value={discountForm.ends_at}
                    onChange={(e) => setDiscountForm((s) => ({ ...s, ends_at: e.target.value }))}
                    className="mt-2 w-full bg-night/60 border border-border/60 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
                  />
                </label>
                <label className="flex items-center gap-2 text-sm text-foreground/70 sm:col-span-2">
                  <input
                    ...
                    onChange={(e) => setDiscountForm((s) => ({ ...s, active: e.target.checked }))}
                ...
                <button
                  ...
                  disabled={saving || !discountForm.name}
        ...
        <div className="glass rounded-3xl overflow-hidden">
          <div className="p-5 sm:p-6 border-b border-border/20">
            ...
            <p className="text-xs text-foreground/50 mt-1">
              Toggle whether each event requires payment. Free events won't show a "Buy Ticket"
              button.
            </p>
          </div>
          <div className="overflow-x-auto">
              ...
              <tbody>
                {events.map((e) => (
                  <tr key={e.id} className="border-t border-border/30 hover:bg-white/[0.02]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-1 rounded-full text-[10px] uppercase tracking-wider ${e.status === "published" ? "bg-savanna/15 text-savanna" : "bg-foreground/10 text-foreground/60"}`}
                      >
                        {e.status}
                      </span>
                    </td>
