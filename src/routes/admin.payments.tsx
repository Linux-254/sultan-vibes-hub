      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-foreground/60 mt-1">
            ...
            <span className="text-gold font-medium">KES {totals.toLocaleString()}</span>
          </p>
        </div>
                ...
                <tr key={r.id} className="border-t border-border/30 hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      {r.status === "pending" && (
                        <IconBtn label="Mark success" onClick={() => update(r.id, "success")}>
                          <Check size={14} />
                        </IconBtn>
                      )}
                      {r.status === "pending" && (
                        <IconBtn label="Mark failed" onClick={() => update(r.id, "failed")}>
                          <X size={14} />
                        </IconBtn>
                      )}
                      {r.status === "success" && (
                        <IconBtn label="Refund" onClick={() => update(r.id, "refunded")}>
                          <RotateCcw size={14} />
                        </IconBtn>
                      )}
                    </div>
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-foreground/50 text-sm">
                    No payments yet. Once Daraja STK Push is wired in M3, transactions will land
                    here automatically.
                  </td>
                </tr>
              )}
