...
export const Route = createFileRoute("/talent")({
  head: () => ({
    meta: [
      { title: "Talent — Empire Kwa Sultan" },
      {
        name: "description",
        content:
          "Empire scouts and books DJs, artists, MCs, dancers, photographers and videographers across Nairobi.",
      },
      {
        name: "keywords",
        content:
          "Nairobi DJs, talent Kenya, MC Nairobi, live artist Kenya, photographer Nairobi, videographer Nairobi, Empire Kwa Sultan talent",
      },
...
function TalentPage() {
  ...
  const visible = tab === "All" ? roster : roster.filter((r) => r.talent_type === tab);
    ...
    <>
      <section className="mx-auto max-w-7xl px-5 lg:px-8 py-20">
        ...
        <p className="mt-5 max-w-xl text-foreground/70">
          DJs, artists, MCs, dancers, photographers and videographers — Empire books fresh talent
          every week. Submit your work, we review and respond.
        </p>
        ...
        {loading ? (
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="glass rounded-3xl p-5 animate-pulse h-36" />
            ))}
          </div>
        ) : (
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {visible.map((r) => {
              const Icon = ICONS[r.talent_type] ?? Music2;
              ...
              return (
                <Link
                  ...
                  className="group glass rounded-3xl p-5 hover:border-gold/40 transition"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-display text-lg truncate">{r.stage_name}</div>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <span
                      className={`text-xs ${available ? "text-savanna" : "text-foreground/55"}`}
                    >
                      {r.status}
                    </span>
                    <span className="text-xs text-gold inline-flex items-center gap-1">
                      View <ArrowRight size={12} className="transition group-hover:translate-x-1" />
                    </span>
                  </div>
                </Link>
              );
      ...
      <section className="mx-auto max-w-7xl px-5 lg:px-8 pb-20">
        <div className="glass rounded-3xl p-8 lg:p-12 kente-border">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {["Alcapone", "Bag DJs", "Steph Kapela", "Bensoul", "Karun", "Wakadinali"].map((b) => (
              <span
                key={b}
                className="px-4 py-2 rounded-full border border-gold/30 text-foreground/80 font-display text-sm hover:bg-gold/10 transition"
              >
                {b}
              </span>
            ))}
      ...
      <section className="mx-auto max-w-4xl px-5 lg:px-8 pb-32">
        <div className="glass rounded-3xl p-7 lg:p-10">
          ...
          <h3 className="font-display text-3xl mt-3">
            Tell us why <span className="text-gold-gradient">Empire</span>.
          </h3>
          <form onSubmit={(e) => e.preventDefault()} className="mt-8 grid sm:grid-cols-2 gap-3">
            <I label="Stage name" />
            <S
              label="Talent type"
              options={TYPES.filter((t) => t !== "All") as unknown as string[]}
            />
            ...
            <div className="sm:col-span-2">
              <label className="block">
                ...
                <textarea
                  maxLength={200}
                  rows={3}
                  className="mt-1.5 w-full bg-night/60 border border-border/50 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold resize-none"
                />
              </label>
            </div>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="mt-1.5 w-full bg-night/60 border border-border/50 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:border-gold"
      />
    </label>
  );
}
