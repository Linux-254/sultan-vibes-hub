...
function UserChatPage() {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    {
      id: "1",
      from: "admin",
      body: "Karibu Empire. This line goes straight to admin — bookings, collabs, anything.",
  ...
  const send = (e: FormEvent) => {
    ...
    setMsgs((m) => [
      ...m,
      { id: String(Date.now()), from: "you", body: text.trim(), at: new Date().toISOString() },
    ]);
    setText("");
  };
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="font-display text-3xl mt-2">
            Direct line to <span className="text-gold-gradient">admin</span>
          </h1>
          <p className="text-xs text-foreground/55 mt-1">
            All chat is admin-mediated. No DMs between users by default.
          </p>
        </div>
          ...
          {msgs.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${mine ? "bg-gold text-night-deep" : "bg-white/[0.06]"}`}
                >
                  {!mine && (
