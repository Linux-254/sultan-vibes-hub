                ...
                <tr key={r.id} className="border-t border-border/30 hover:bg-white/[0.02]">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      {r.status === "pending" && (
                        <IconBtn label="Approve" onClick={() => update(r.id, "approved")}>
                          <Check size={14} />
                        </IconBtn>
                      )}
                      {(r.status === "pending" || r.status === "approved") && (
                        <IconBtn label="Cancel" onClick={() => update(r.id, "cancelled")}>
                          <X size={14} />
                        </IconBtn>
                      )}
                      {r.status === "approved" && (
                        <IconBtn label="Seat" onClick={() => update(r.id, "seated")}>
                          <UtensilsCrossed size={14} />
                        </IconBtn>
                      )}
                      {r.status === "approved" && (
                        <IconBtn label="No-show" onClick={() => update(r.id, "no_show")}>
                          <Ban size={14} />
                        </IconBtn>
                      )}
                    </div>
