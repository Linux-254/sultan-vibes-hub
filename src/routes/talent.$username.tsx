...
export const Route = createFileRoute("/talent/$username")({
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `${prettify(params.username)} on the Empire Kwa Sultan talent roster.`,
      },
    ],
  }),
      ...
      <div className="grid lg:grid-cols-[200px_1fr] gap-8 items-start">
        ...
        <div>
          ...
          <p className="mt-4 text-foreground/70 max-w-xl">
            Resident sound. Catch their next set on the Empire calendar or book them direct via
            admin.
          </p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link
              to="/chat"
              className="rounded-full bg-gold px-5 py-2.5 text-sm font-semibold text-night-deep inline-flex items-center gap-1.5"
            >
              <MessageCircle size={14} /> Book {name}
            </Link>
            <Link
              to="/events"
              className="rounded-full border border-border/40 px-5 py-2.5 text-sm hover:border-gold"
            >
              See upcoming sets
            </Link>
          </div>
